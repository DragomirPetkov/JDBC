...
public class BeerRepositoryImpl implements BeerRepository {
    ...
    public BeerRepositoryImpl(StyleRepository styleRepository, UserRepository userRepository) {
        ...
        beer.setCreateBy(userRepository.getById(1));
        beers.add(beer);

        beer = new Beer(2, "Rhombus Porter", 5.0);
        beer.setStyle(styleRepository.get(2));
        beer.setCreateBy(userRepository.getById(2));
        beers.add(beer);

        beer = new Beer(3, "Opasen Char", 6.6);
        beer.setStyle(styleRepository.get(3));
        beer.setCreateBy(userRepository.getById(3));
        beers.add(beer);
    }

    @Override
    public List<Beer> getByName(String name, Double minAbv, Double maxAbv, Integer styleId, String sortBy, String sortOrder) {
    ...
    public void update(Beer beer) {
        Beer beerToUpdate = getByName(beer.getId());
    ...
    public void delete(int id) {
        Beer beerToDelete = getByName(id);
